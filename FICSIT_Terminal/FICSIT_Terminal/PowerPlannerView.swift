                ...
                ScrollView {
                    VStack(spacing: 25) {
                        VStack(alignment: .leading) {
                            Text("MONITEUR DE RÉSEAU").font(.headline).foregroundColor(.ficsitOrange).padding(.top)
                            Text("Équilibrer production vs consommation.").font(.caption).foregroundColor(.gray)
                        }.frame(maxWidth: .infinity, alignment: .leading).padding(.horizontal)
                        
                        VStack(spacing: 15) {
                            HStack { Text("TYPE DE CARBURANT").font(.caption).fontWeight(.bold).foregroundColor(.gray); Spacer() }
                            ...
                            HStack { Text("QUANTITÉ DISPONIBLE").font(.caption).fontWeight(.bold).foregroundColor(.gray); Spacer() }
                            HStack {
                            ...
                            Button(action: { withAnimation { viewModel.calculatePower() } }) { HStack { Image(systemName: "bolt.fill"); Text("METTRE À JOUR RÉSEAU") }.font(.headline).frame(maxWidth: .infinity).padding().background(Color.ficsitOrange).foregroundColor(.black).cornerRadius(10) }
                        ...
                        if let result = viewModel.powerResult {
                            VStack(spacing: 20) {
                                ...
                                ZStack {
                                    ...
                                    VStack(spacing: 5) {
                                        Text(isOverloaded ? "SURCHARGE" : "STABLE").font(.caption).fontWeight(.black).foregroundColor(isOverloaded ? .red : .green).padding(5).background(Color.black.opacity(0.5)).cornerRadius(5)
                                        ...
                                        Text("Charge: \(Int(loadPercentage * 100))%").font(.caption).foregroundColor(.ficsitOrange)
                                    }.offset(y: 10)
                                ...
                                VStack(spacing: 15) {
                                    HStack {
                                        ...
                                        VStack(alignment: .leading) { Text(result.fuel.generatorType).font(.headline).foregroundColor(.white); Text("Conso: \(String(format: "%.1f", 60/result.fuel.burnTime))/min").font(.caption).foregroundColor(.gray) }
                                    ...
                                    if result.fuel == .coal {
                                        HStack {
                                            ...
                                            VStack(alignment: .leading) { Text("Eau Requise").font(.headline).foregroundColor(.white); Text("\(Int(result.waterNeeded)) m³/min").font(.caption).foregroundColor(.blue) }
                                            Spacer(); VStack(alignment: .trailing) { Text("\(String(format: "%.1f", result.waterExtractors))x").font(.title2).fontWeight(.bold).foregroundColor(.blue); Text("Extracteurs").font(.caption).foregroundColor(.gray) }
                                        }
                                    }
                                    HStack {
                                        Image(systemName: isOverloaded ? "exclamationmark.triangle.fill" : "battery.100.bolt").foregroundColor(isOverloaded ? .red : .green)
                                        if isOverloaded { Text("Panne imminente! Ajoutez \(Int(consumption - production)) MW.").font(.caption).foregroundColor(.red) }
                                        else { Text("Capacité de réserve: \(Int(production - consumption)) MW").font(.caption).foregroundColor(.green) }
                        ...
                        } else { Text("Entrez une quantité de carburant pour simuler.").foregroundColor(.gray).padding(.top, 40) }
                        Spacer()
                    }
                }
